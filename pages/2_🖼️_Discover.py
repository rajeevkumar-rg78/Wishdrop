...
st.caption(f"Showing {len(df)} items for **{chosen}** (optimized for mobile view).")
...
for i, row in df.reset_index(drop=True).iterrows():
    with cols[i % 2]:
        # Smaller Unsplash image (loads faster on mobile)
        img_url = row["image_url"].replace("800x1000", "400x500")
        st.image(img_url, use_container_width=True)
        st.markdown(f"**{row['name']}**")
        st.caption(f"{row['brand']} • {row['category']} • {row['store']}")
        st.markdown(
            f"${row['price']:,.2f}  ~  ~~${row['msrp']:,.2f}~~  •  **-{int(row['discount_pct'])}%**"
        )
        ...
        # Lazy-load chart
        with st.expander("📉 View Price Trend (optional)"):
